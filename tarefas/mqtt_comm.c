/**
 * @file mqtt_comm.c
 * @brief Implementação das funções de comunicação MQTT básica
 */
...
#include "lwip/ip4_addr.h"        // Para usar ip4addr_aton
...
void mqtt_setup(const char *client_id, const char *broker_ip, const char
*user, const char *pass) {
    ...
    if (!ip4addr_aton(broker_ip, ip_2_ip4(&broker_addr))) {
        printf("Erro no IP\n");
        return;
    }
    ...
    ci.client_user = user;
    ci.client_pass = pass;
    ci.keep_alive = 60; // segundos
    ci.will_topic = NULL; // sem will topic
    ci.will_msg = NULL;
    ci.will_qos = 0;
    ci.will_retain = 0;
    ...
    mqtt_client_connect(client, &broker_addr, 1883, mqtt_connection_cb,
NULL, &ci);
}
